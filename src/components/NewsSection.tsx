...
export default function NewsSection(props: NewsSectionProps) {
  ...
  useEffect(() => {
    ...
    if (newsCards.length > 0) {
      currentCardRef.current = document.getElementById(newsCards[0]?._id) as HTMLDivElement;
    }
  }, [newsCards]);
  ...
  const nextSlide = () => {
    ...
    if (currentCardRef.current) {
      ...
      if (nextCard) {
        ...
        const firstCard = document.getElementById(newsCards[0]._id) as HTMLDivElement;
        if (firstCard) {
    ...
    if (currentCardRef.current) {
      ...
      if (prevCard) {
        ...
        const lastCard = document.getElementById(
          newsCards[newsCards.length - 1]._id,
        ) as HTMLDivElement;
        if (lastCard) {
