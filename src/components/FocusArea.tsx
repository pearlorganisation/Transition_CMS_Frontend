    ...
    <div className="card bg-base-100 shadow-lg border-2 w-full h-auto">
      <div className="card-body flex flex-col">
        ...
        <button
          ...
          className="mt-auto text-2xl px-0 text-primary self-start"
        >
...
console.log("the focus data is", focusData)
  ...
  useEffect(() => {
    ...
    if (focusCards.length > 0) {
      currentCardRef.current = document.getElementById(focusCards[0].name) as HTMLDivElement;
    }
  }, []);
  ...
  const nextSlide = () => {
    if (currentCardRef.current) {
      ...
      if (nextCard) {
        ...
        const firstCard = document.getElementById(focusCards[0].name) as HTMLDivElement;
        if (firstCard) {
    ...
    if (currentCardRef.current) {
      ...
      if (prevCard) {
        ...
        const lastCard = document.getElementById(
          focusCards[focusCards.length - 1].name,
        ) as HTMLDivElement;
        if (lastCard) {
